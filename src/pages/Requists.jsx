...
const Requists = () => {
	...
	const [ordersConv, setOrdersConv] = useState([])
	const [ordersLate, setOrdersLate] = useState([])
	const [ordersReady, setOrdersReady] = useState([])
	const [isModalOpen, setIsModalOpen] = useState(false);
	const [activeOrder, setActiveOrder] = useState(0)
	const [files, setFiles] = useState([])
	const [getRuquests, getRuquestsIsLoading] = useApi({
		url: '/architectOrders/getOrdersFilter',
		data: {
			countElementOnPage: paginationState.inWork.itemsPerPage,
			numPage: (paginationState.inWork.currentPage - 1),
			state: 'IN_WORK'
		}
	});
	const [getRuquestsConv, getRuquestsIsLoadingConv] = useApi({
		url: '/architectOrders/getOrdersFilter',
		data: {
			countElementOnPage: paginationState.conv.itemsPerPage,
			numPage: (paginationState.conv.currentPage - 1),
			state: 'DIALOG_CREATED'
		}
	});
	const [getRuquestsLate, getRuquestsIsLoadingLate] = useApi({
		url: '/architectOrders/getOrdersFilter',
		data: {
			countElementOnPage: paginationState.late.itemsPerPage,
			numPage: (paginationState.late.currentPage - 1),
			state: 'LATE'
		}
	});
	const [getRuquestsReady, getRuquestsIsLoadingReady] = useApi({
		url: '/architectOrders/getOrdersFilter',
		data: {
			countElementOnPage: paginationState.ready.itemsPerPage,
			numPage: (paginationState.ready.currentPage - 1),
			state: 'READY'
		}
	});
	const [tryFinishOrder, finishOrderIsLoading] = useApi({
		url: '/architectOrders/changeStateOfOrderReady',
		data: {
			filesId: files,
			orderId: activeOrder,
		}
	...
	useEffect(() => {
		...
		const addTime = (elt) => {
			let el = {...elt };
			el.timeLeft = {};
			let date1 = new Date(el.timeOfChangeState);
		...
		getCurTime().then((resp) => {
			...
			getRuquests().then((resp) => {
				setOrders(resp.orderResponseList.map((elt) => addTime(elt)));
				setOrdersCount((st) => { return {...st, inWork: resp.allCount}})
			}).catch((e) => setServerError(e.message))
		})
		getRuquestsConv().then((resp) => {
			setOrdersConv(resp.orderResponseList.map((elt) => addTime(elt)))
			setOrdersCount((st) => { return {...st, conv: resp.allCount}})
		}).catch((e) => setServerError(e.message))
		getRuquestsLate().then((resp) => {
			setOrdersLate(resp.orderResponseList.map((elt) => addTime(elt)))
			setOrdersCount((st) => { return {...st, late: resp.allCount}})
		}).catch((e) => setServerError(e.message))
		getRuquestsReady().then((resp) => {
			setOrdersReady(resp.orderResponseList)
			setOrdersCount((st) => { return {...st, ready: resp.allCount}})
		}).catch((e) => setServerError(e.message))
	},[paginationState])

	const tick = () => {
		if (!getRuquestsIsLoading) {
			setOrders(orders.map((ord) => {
				...
				if (ord.timeLeft.m === 0 && ord.timeLeft.s === 0) {
					tm.h = ord.timeLeft.h - 1;
					tm.m = 59;
					tm.s = 59;
				} else if (ord.timeLeft.s === 0) {
					tm.m = ord.timeLeft.m - 1;
					tm.s = 59;
				} else {
					tm.s = ord.timeLeft.s - 1;
				}
				return {...ord, timeLeft: tm }
			}))
		}
		if (!getRuquestsIsLoadingConv) {
			setOrdersConv(ordersConv.map((ord) => {
				let tm = {
					h: ord.timeLeft.h,
					m: ord.timeLeft.m,
					s: ord.timeLeft.s
		...
		if (!getRuquestsIsLoadingLate) {
			setOrdersLate(ordersLate.map((ord) => {
				let tm = {
					h: ord.timeLeft.h,
					m: ord.timeLeft.m,
					s: ord.timeLeft.s
				...
				if (ord.timeLeft.m === 59 && ord.timeLeft.s === 59) {
					tm.h = ord.timeLeft.h + 1;
					tm.m = 0;
					tm.s = 0;
				} else if (ord.timeLeft.s === 59) {
					tm.m = ord.timeLeft.m + 1;
					tm.s = 0;
				} else {
					tm.s = ord.timeLeft.s + 1;
	...
	const handleChange = (info) => {
		...
		newFileList = newFileList.map((file) => {
			if (file.response) {
				...
				setFiles([...files, file.response[0].id])
			}
			return file;
		});
	...
	const props = {
		name: 'files',
		multiple: true,
		action: hostName + '/architectOrders/addArchitectFiles',
		headers: {
	...
	const finishOrder = () => {
		if (files.length) {
			tryFinishOrder().then(() => {
				closeModal()
				setPaginationState({...paginationState, updatePage: Math.floor(Math.random() * 100000)})
			})
		}
	}

	const [chatModalIsOpen, setChatModalIsOpen] = useState(false)
	const [curDialogId, setCurDialogId] = useState()
	const callModalChat = (dialogId) => {
		setCurDialogId(dialogId);
	...
	useEffect(() => {
		if (curDialogId) {
			getRoom(curDialogId).then(roomData => {
	...
	const selfNewMess = (text) => {
		AddNewMessage({ content: text, dialogId: curDialogId, uniqueCode: uuidv4() });
	}
	const propsUpl = {
		name: 'files',
	...
	return (
		<div className="admin-container">
			...
			<Tabs className="admin-tabs" defaultActiveKey="1">
				...
				<Tabs.TabPane tab={ordersCount.conv !== null ? "Обсуждение " + ordersCount.conv : 'Обсуждение' } key="2">
					{getRuquestsIsLoadingConv ?
						<Loading />
						:
						<>
							{ordersConv.length ?
				...
				<Tabs.TabPane tab={ordersCount.late !== null ? "Просрочено " + ordersCount.late : 'Просрочено' } key="3">
					{getRuquestsIsLoadingLate ?
						<Loading />
						:
						<>
							{ordersLate.length ?
								<>
									...
									<Pagination current={paginationState.late.currentPage} total={ordersCount.late} pageSize={paginationState.late.itemsPerPage} onChange={
										(page, pageSize) => {
											setPaginationState({...paginationState, late: { currentPage: page, itemsPerPage: pageSize }})
										}
									} showSizeChanger/>;
								</>
								:
				...
				<Tabs.TabPane tab={ordersCount.ready !== null ? "Завершено " + ordersCount.ready : 'Завершено' } key="4">
					{getRuquestsIsLoadingReady ?
						<Loading />
						:
						<>
							{ordersReady .length ?
								<>
									...
									<Pagination current={paginationState.ready.currentPage} total={ordersCount.ready} pageSize={paginationState.ready.itemsPerPage} onChange={
										(page, pageSize) => {
											setPaginationState({...paginationState, ready: { currentPage: page, itemsPerPage: pageSize }})
										}
									} showSizeChanger/>;
								</>
								:
			...
			<Modal className="upload-modal" title="Завершить заказ?" visible={isModalOpen} onCancel={closeModal}>
				<Dragger {...props} listType="picture" fileList={fileList}>
				...
				<div className="upload-modal-actions">
					<div className="preview-list__btn" onClick={finishOrder}>{finishOrderIsLoading && <LoadingOutlined />}Отправить</div>
					<div className="preview-list__btn" onClick={closeModal}>Отмена</div>
				</div>
			</Modal>
			<Modal className="chat-modal" title={ "Чат " + curDialogId } visible={chatModalIsOpen} onCancel={closeModalChat}>
								...
								<div className="chat-room__container">
									<div className="chat-room__title">
										{roomIsLoading
											...
											:
											<>
												<div className="chat-room__title-name">{roomName}</div>
											</>
										}
									</div>
